...
$(function() {
   model = ko. observableArray([{
        ...
        lessCommon: ["Murphy Brown", "Lois and Clark", "Rent", "Road to Perdition", "Dick Tracy", "A Little Princess", "Entourage", "Michael Jackson Pepsi Commercial", "LMFAO Party Rock Anthem", "Two Broke Girls", "J Edgar", "Jersey Boys", "Angels with Dirty Faces", "Cloverfield"],
        ...
        lessCommon: ["Postcards from the Edge", "ER", "The West Wing", "Murphy Brown", "Annie (1999)", "Two of a Kind", "Pretty Little Liars"],
    ...
    viewModel = function() {
        ...
        self.chooseRandom = function() {
            var number = Math.floor(Math.random(model().length * 1) *10);
            var area = model()[number];
            self.lotArea(area);
            console.log(area);
            self.areaName(area.name);
            self.areaBuilt(area.built);
            self.areaCommon(area.common);
            self.areaLessCommon(area.lessCommon);
